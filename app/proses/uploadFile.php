...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['action']) && $_GET['action'] == 'create') {
    ...
    if (!empty($_FILES['file']['name'])) {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)) {
            ...
            $sql = "INSERT INTO product (seller_id, name, description, price, created_at, product_type, kategori_id)
                    VALUES ('$sellerId', '$name', '$description', '$price', CURRENT_TIMESTAMP, '$productType', '$kategoriId')";
            $sql2 = "INSERT INTO product_file (seller_id, file_name, file_path) 
            VALUES ('$sellerId', '$fileName', '$targetFilePath')";
